...
testthat::test_that('Test the prediction functions in RCP',{
  ...
  bootmod <- regional_mix_boot(fm,nboot = 10)
  bootmod1 <- regional_mix_boot(fm,nboot = 10,type = "SimpleBoot")
  predsboot <- predict(fm, bootmod)
  predsboot1 <- predict(fm, bootmod1)
  ...
  preds_new <- predict(fm,newdata = xwnew)
