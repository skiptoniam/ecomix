...
form <- as.formula(paste0("cbind(",paste(sp_names[1:127],collapse = ','),")~1+min_temp+min_temp2+max_temp+max_temp2+annual_rainfall+annual_rainfall2+soil_type+soil_type2"))

colnames(wts)<-sp_names

# fm4_myrt_var <- species_mix(archetype_formula = form, data = dat, weights = wts,
#                             n_mixtures = 4, distribution = 'ippm', standardise = TRUE,
#                             control = species_mix.control(cores = 1))
...
for (i in 1:S){
  print(i)
  fm_sp_mods[[i]] <- try(ecomix:::apply_glm_sam_inits(i, y, X, site_spp_weights, offset, y_is_na, disty))
                                      # .parallel = control$cores, .verbose = FALSE)# !control$quiet)
}

fm_sp_mods2 <-  surveillance::plapply(seq_len(S), try(ecomix:::apply_glm_sam_inits), y, X, site_spp_weights, offset, y_is_na, disty,
                                      .parallel = control$cores, .verbose = FALSE)
fmalpha <- unlist(lapply(fm_sp_mods, `[[`, 1))
