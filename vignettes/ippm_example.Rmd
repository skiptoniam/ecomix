...
form <- as.formula(paste0("cbind(",paste(sp_names[1:100],collapse = ','),")~1+poly(min_temp,degree=2)+poly(max_temp,degree=2)+poly(annual_rainfall,degree=2)"))
...
fm1 <- species_mix(archetype_formula = form, data = dat, weights = wts,
                            ...
                            control = species_mix.control(init_method = 'kmed',cores = 1,
                                                          minimum_sites_prevelance = 0.025))
df <- as.data.frame(covar[[-4]])
df_no_na <- na.omit(df)
