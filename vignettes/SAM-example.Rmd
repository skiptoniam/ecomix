...
```{r, eval = TRUE, fig.width=5,fig.height=4,fig.cap="Figure 3. Group selection from multiple fit function, we can see that three archetypes is the best fit to these data based on Bayesian Information Criterion (BIC)."}
nArchetypes <- 1:6
...
```{r,fig.width=8, fig.height=6, fig.cap="Figure 4. BIC from species archetype model fits per group."}
...
```{r, fig.width=8, fig.height=6, fig.cap="Figure 5. Random quantile residuals for all species in the fitted model."}
plot(sam_fit,fitted.scale = 'logit')
```

We can also look at the random quantile residuals for a single species
```{r, fig.width=8, fig.height=6, fig.cap="Figure 6. Random quantile residuals for spp3 from the fitted model."}
...
```{r partial plots, fig.width=6, fig.height=7, fig.cap="Figure 6. Partial response plots for each covariate in the model."}
par(mfrow=c(2,3))
...
```{r archetype predictions,echo=FALSE,fig.width= 4, fig.height= 8,fig.cap="Figure 8. The predicted probability of each species archetype across the simulated environment and the standard error of the predictions generated based on the Bayesian bootstrap."}
