...
extern "C" { SEXP species_mix_cpp(SEXP R_pars, SEXP R_y, SEXP R_X, SEXP R_ID,SEXP R_tau, SEXP R_gradient, SEXP R_offset, SEXP R_model_type){
    ...
    // model_type selects model, 1 = binomial, 2 = negative binomial, 3 = tweedie
 ...
 * And setups data classes for negative binomial.  
 * 
 * 
 */
...
double optimise_nbinom(int n, double *pars, void *ex){
  ...
  logl.at(0) = negative_binomial_logl(x,*data);
...
double negative_binomial_logl(vector<double> &pars, Optimise_data_nbinom &data ){//vector<double> &tau, double theta, vector<double> &X, vector<double> &y, double offset){
...
double optimise_mix_nbinom_function(int n, double *pars, void *ex){
...
void gradient_mixnbinom_function(int n, double *pars, double *gr, void *ex ){
 ...
 vector <double> calc_mix_nbinom_logl(const vector<double> &pars, Optimise_data &data){
  int G,S,Xr,Xc;
  G=data.G;
  S=data.S;
  Xr=data.Xr;
 ...
 * This should be very similar to the negative binomial with y_is_na added in and z 
 *
 * Ippm external calls
 * and setups data classes for negative binomial.   
 */
