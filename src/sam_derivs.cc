...
spmix_derivs::spmix_derivs(){};
spmix_derivs::~spmix_derivs(){};

void spmix_derivs::setVals( const myData &dat, SEXP &RderivsAlpha, SEXP &RderivsTau, SEXP &RderivsBeta, SEXP &RderivsGamma, SEXP &RderivsDisp, SEXP &RgetScores, SEXP &Rscores)
{
...
void spmix_derivs::zeroDerivs( const myData &dat)
{
...
void spmix_derivs::updateDerivs( const myData &dat, const vector<double> &alphaDerivsI, const vector<double> &tauDerivsI, const vector<double> &betaDerivsI, const vector<double> &gammaDerivsI, const vector<double> &dispDerivsI, const int &i)
{
...
void spmix_derivs::update( double *grArr, const myData &dat)
{
	int kount=0;
	for( int i=0; i<dat.nS; i++){
		Alpha[i] = grArr[kount];
...
void spmix_derivs::getArray( double *grArr, const myData &dat)
{
	int kount=0;
	for( int i=0; i<dat.nS; i++){
		grArr[kount] = Alpha[i];
